...
| `RELAY_PORT`              | Upstream relay port (587 for STARTTLS, 465 for implicit TLS). 465 requires a mandatory `SMTP_TLS_SECURITY_LEVEL` (`encrypt` or stronger; `none`, `may`, and `dane` — opportunistic without TLSA records — are rejected).                                                                                                                                                                                                                                                                                                                                                                                                                                   | `587`                                     | No       |
| `SMTP_TLS_SECURITY_LEVEL` | Outbound TLS level: `secure` (default; chain + hostname verification), `verify`, `encrypt`, `dane`/`dane-only`/`fingerprint`, `may`, or `none`. See the [Postfix TLS README](https://www.postfix.org/TLS_README.html). Prefer `secure`/`verify` when SASL (`RELAY_LOGIN`/`RELAY_PASSWORD`) is set; `encrypt` and weaker lack peer authentication. `dane`/`dane-only` need a DNSSEC-validating resolver and `smtp_dns_support_level = dnssec`, which this image does not render — `dane` degrades to `may` and `dane-only` defers all mail; `fingerprint` requires `smtp_tls_fingerprint_cert_match`, which is not configurable — prefer `secure`/`verify`. | `secure`                                  | No       |
