...
#
# compatibility_level tracks the pinned Postfix major.minor: a lower level changes
# no effective setting here, because this body renders every value the level would
# otherwise default, but it makes every startup invocation print the
...
# common deploy-time faults (DNS, routing, wrong port, firewall) for both STARTTLS
# and implicit TLS, and it verifies neither SASL nor the TLS chain, which only a
# real send proves and deferred-queue alerting surfaces. A peer that accepts and
# then goes silent is bounded by the outer timeout, not by nc's own -w: BusyBox nc
# retries that idle poll twice (netretry = 2), so its own close needs 2x -w, which
# is 20s at the top of the validated range. The SIGTERM at -w + 2 is what holds the
# probe under the 15s healthcheck start-period (max 10 + 2 = 12s; a TERM-ignoring
# nc is KILLed 2s later), and it lands as the fail-soft warn.
probe_relay_tcp() {
  ...
  # Nothing is sent: a command before the greeting is a pipelining violation the
  # upstream logs against us. stdin at EOF half-closes instead, which a peer
  # answers by closing, so the verdict still lands in well under a second.
  timeout -k 2 "$((_probe_timeout + 2))" nc -w "$_probe_timeout" "$1" "$2" </dev/null >/dev/null 2>&1
}
